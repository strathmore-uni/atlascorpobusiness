...
//////////////////
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_LOCAL}/api/admin/orders/orders`, {
          params: { 
            email: adminEmail, 
    ...
    fetchOrders();
  }, [ adminEmail, selectedCountry, startDate, endDate, currentPage]);
    ...
    <div className="order-details-page">
      ...
      <ul> 
        {filteredOrders.slice((currentPage - 1) * ordersPerPage, currentPage * ordersPerPage).map(order => (
          <li key={order.id}>
            <Link to={`/finaceordertails/${order.id}`} className="order-details-link">
