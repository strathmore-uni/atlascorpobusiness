...
const WarehouseInventoryReport = () => {
  ...
  const fetchInventoryData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/warehouse/inventory-report');
      if (!response.ok) {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ['Metric', 'Value', 'Change'],
      ['Total SKUs', data.totalSkus, data.skuGrowth],
      ['Low Stock Items', data.lowStockItems, data.lowStockChange],
      ['Out of Stock Items', data.outOfStockItems, data.outOfStockChange],
      ['Inventory Categories', '', ''],
      ...data.inventoryCategories.map(item => [item.category, item.count, item.percentage]),
    ...
    a.href = url;
    a.download = 'warehouse-inventory-report.csv';
    a.click();
  ...
  const categoriesData = {
    labels: data.inventoryCategories.map(item => item.category),
    datasets: [
      {
        data: data.inventoryCategories.map(item => item.count),
        backgroundColor: [
          '#3B82F6',
          '#10B981',
          '#F59E0B',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-3xl font-bold text-gray-900">Warehouse Inventory Report</h1>
            <button
              onClick={exportToCSV}
          ...
          <p className="text-gray-600">Comprehensive inventory analysis and stock management</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <p className="text-2xl font-bold text-gray-900">{data.totalSkus.toLocaleString()}</p>
            <p className={`text-sm ${data.skuGrowth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {data.skuGrowth >= 0 ? '+' : ''}{data.skuGrowth}% from last period
            </p>
          </div>
          <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Inventory Categories Details</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data.inventoryCategories.map((category, index) => (
                  <tr key={index}>
...
export default WarehouseInventoryReport; 